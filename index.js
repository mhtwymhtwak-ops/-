...
if (!BOT_TOKEN) { console.error('❌  BOT_TOKEN is required. Set it as an environment variable.'); process.exit(1); }
if (!ADMIN_ID || Number.isNaN(ADMIN_ID)) { console.error('❌  ADMIN_ID is required. Set it as an environment variable.'); process.exit(1); }
...
async function checkNumbers(state, chatId, numbers, userId) {
  ...
  if (registered.length > 0) {
    ...
    for (let i = 0; i < rows.length; i += 20) {
      const isLast = i + 20 >= rows.length;
      await bot.sendMessage(chatId,
        i === 0 ? `✅ <b>الأرقام المسجلة [ ${registered.length} ]</b>\nاضغط على أي رقم لنسخه:` : '↓',
  ...
  if (unregistered.length > 0) {
    ...
    for (let i = 0; i < rows.length; i += 20) {
      await bot.sendMessage(chatId,
        i === 0 ? `❌ <b>الأرقام غير المسجلة [ ${unregistered.length} ]</b>\nاضغط على أي رقم لنسخه:` : '↓',
        { parse_mode: 'HTML', reply_markup: { inline_keyboard: rows.slice(i, i + 20) } }
      );
    }
  }
  ...
  if (errors.length > 0) {
    await bot.sendMessage(chatId, `⚠️ أخطاء في الفحص: ${errors.length} رقم`);
  }

  await bot.sendMessage(chatId,
    `📊 <b>ملخص الفحص</b>\n✅ مسجل: ${registered.length} | ❌ غير مسجل: ${unregistered.length}`,
    { parse_mode: 'HTML', reply_markup: afterCheckKeyboard(userId, state.checks.totalChecked, hasPending) }
  );
}
...
http.createServer((req, res) => {
  ...
  console.log(`🌐 Keep-alive server على البورت ${PORT}`);
});
